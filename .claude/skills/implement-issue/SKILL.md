...
This is the **Claude Code** copy of the skill, at
`.claude/skills/implement-issue/` — Claude Code auto-discovers project skills
under `.claude/skills/` and exposes this one as the `/implement-issue` command.
A parallel **GitHub Copilot** copy lives at `.github/skills/implement-issue/`
(entry point `/implement-issue <N>` via `.github/prompts/implement-issue.prompt.md`).
The two are independent copies — keep them in sync when either changes.
...
```bash
node .claude/skills/implement-issue/ci-parity.mjs              # all 24 steps
node .claude/skills/implement-issue/ci-parity.mjs lint test    # subset by id
node .claude/skills/implement-issue/ci-parity.mjs --list       # step ids
```
