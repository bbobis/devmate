...
/**
 * Resolve the analysis-dispatch sequencing mode from a config object.
 * `dispatchSequencing` ('off' | 'warn' | 'block') selects how an out-of-order
 * analysis dispatch (an analysis agent dispatched before its prerequisite gate,
 * per lib/workflow/dispatch-sequencing.mjs) is handled at the PreToolUse
 * gate-guard. Defaults to **'warn'** (surface a model-visible advisory, do not
 * block). RC-3, issue #231.
 * @param {unknown} config
 * @returns {'off'|'warn'|'block'}
 */
export function resolveDispatchSequencingMode(config) {
  if (config === null || typeof config !== 'object') return 'warn';
  const rec = /** @type {Record<string, unknown>} */ (config);
  const mode = rec['dispatchSequencing'];
  if (mode === 'off' || mode === 'warn' || mode === 'block') return mode;
  return 'warn';
}
...
export function validateDevmateConfig(raw) {
  ...
  // Analysis-dispatch sequencing mode (default 'warn'). `dispatchSequencing`
  // selects how the PreToolUse gate-guard reacts to an analysis dispatch
  // (rubber-duck / planner / spec-writer) that arrives before its prerequisite
  // internal gate: 'off' (ignore) | 'warn' (model-visible advisory, non-blocking)
  // | 'block' (deny the dispatch). RC-3, issue #231.
  if (obj['dispatchSequencing'] !== undefined &&
      !['off', 'warn', 'block'].includes(/** @type {string} */ (obj['dispatchSequencing']))) {
    return { ok: false, error: "devmate.config.json dispatchSequencing must be one of 'off', 'warn', 'block' when present" };
  }
