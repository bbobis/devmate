...
export async function captureMemory(repoRoot, opts = {}) {
  ...
  // 2) Render .devmate/MEMORY.md from the repo ledger. The render itself always
  //    runs (so facts promoted by earlier tasks stay current), but it is
  //    write-on-change only (#149): a session that promoted nothing new produces
  //    byte-identical output and skips the write, so the tracked file is never
  //    dirtied from a developer's private local ledger. `rendered` reports whether
  //    the file was actually (re)written, not merely that the render step ran.
