...
/**
 * Malformed-line ratio above which a trace is considered corrupt. Mirrors the
 * threshold `scripts/view-trace.mjs` already enforces, so both surfaces agree
 * on what "too corrupt to trust" means.
 * @type {number}
 */
...
/**
 * Resolve whether a single gate's primary artifact is present (and valid).
 * Pure-ish: reads the filesystem synchronously via the audited helpers. Returns
 * `true` for `none`-kind gates (nothing to prove at this gate itself).
 * @param {WorkflowGate} gate
 * @param {string} stateDir
 * @returns {boolean}
 */
function resolveArtifactPresent(gate, stateDir) {
...
export async function checkGateConsistency(state, opts = {}) {
  ...
  const checkpoints = chain.map((g) => {
    ...
    return {
      gate: g,
      present: resolveArtifactPresent(g, stateDir),
      requiresHumanAudit,
