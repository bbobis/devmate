...
const GATE_PRECONDITIONS = {
  ...
  'verification-passed': async (ctx) => {
    ...
    // Ownership: a fresh, passing verify-result.json left by an EARLIER task must
    // not advance this one. The artifact lives at a fixed path, nothing clears it
    // between tasks, and the specDigest guard below is vacuous on the lanes that
    // write no spec (bug/chore, where specDigest is ''), so ownership is the only
    // guard that stops stale evidence there. Mirrors requireArtifact — reject on a
    // taskId MISMATCH only, so a pre-stamp artifact or a taskless caller is never
    // wedged.
    const verifyOwner = artifactTaskId(artifact);
    const verifyAsker = typeof ctx.taskId === 'string' ? ctx.taskId.trim() : '';
    if (verifyOwner !== null && verifyAsker !== '' && verifyOwner !== verifyAsker) {
      missing.push(
        `verify evidence at ${filePath} belongs to task "${verifyOwner}", not "${verifyAsker}" — it is stale evidence from an earlier task. Re-run the verify step.`,
      );
    }
    if (artifact === null) {
