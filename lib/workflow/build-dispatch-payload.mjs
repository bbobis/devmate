 ...
 * @property {'feature'|'bug'|'chore'} [lane]  Issue 238 lane signal; arms the feature-lane alignment fail-closed check. Absent lane means no assertion.
 */
...
/**
 * #238: alignment evidence is pointer-based (TCM-3) — each rendered reason /
 * pointer line is truncated to this cap so the section never pastes source.
 * Mirrors {@link TARGET_AC_TEXT_CAP}. TODO: calibrate — provisional placeholder.
 */
const ALIGNMENT_TEXT_CAP = 120;

/**
 * Render one alignment target as `symbol @ path`, or `[none]` when absent.
 * @param {unknown} target
 * @returns {string}
 */
function renderAlignmentTarget(target) {
  if (target === null || typeof target !== 'object') return '[none]';
...
/**
 * Join a pointer list into a single comma-separated line, dropping blanks.
 * @param {unknown} value
 * @returns {string}
 */
function renderPointerLine(value) {
  if (!Array.isArray(value)) return '';
  return value.filter((p) => typeof p === 'string' && p.trim() !== '').join(', ');
}

/**
 * #238: render each task's codebase-alignment evidence as a bounded,
 * pointer-only prompt section (never source). Returns '' when no task carries
 * a non-empty `alignment` array, so non-feature / pre-alignment dispatches
 * stay byte-identical.
 * @param {object[]} tasks
 * @returns {string}
 */
function buildAlignmentSection(tasks) {
  const carrying = tasks
    .map((task, index) => ({ index, record: /** @type {Record<string, unknown>} */ (task) }))
    .filter(({ record }) => Array.isArray(record.alignment) && record.alignment.length > 0);
  if (carrying.length === 0) return '';

  /** @type {string[]} */
  const lines = ['## Codebase alignment evidence', ''];
  for (const { index, record } of carrying) {
    const description =
      typeof record.description === 'string' ? capAcText(record.description, ALIGNMENT_TEXT_CAP) : '';
    lines.push(`Task ${index + 1}${description ? ` — ${description}` : ''}`);
    for (const raw of /** @type {unknown[]} */ (record.alignment)) {
      const entry = /** @type {Record<string, unknown>} */ (raw);
      const capability = typeof entry.capability === 'string' ? capAcText(entry.capability, ALIGNMENT_TEXT_CAP) : '';
      const decision = typeof entry.decision === 'string' ? entry.decision : '';
      lines.push(`- capability: ${capability} | decision: ${decision}`);
      lines.push(`  target: ${capAcText(renderAlignmentTarget(entry.target), ALIGNMENT_TEXT_CAP)}`);
      const usage = renderPointerLine(entry.usageEvidence);
      if (usage !== '') lines.push(`  usage: ${capAcText(usage, ALIGNMENT_TEXT_CAP)}`);
      const pattern = renderPointerLine(entry.patternRefs);
      if (pattern !== '') lines.push(`  pattern: ${capAcText(pattern, ALIGNMENT_TEXT_CAP)}`);
      if (typeof entry.reason === 'string' && entry.reason.trim() !== '') {
        lines.push(`  reason: ${capAcText(entry.reason, ALIGNMENT_TEXT_CAP)}`);
      }
    }
  }
  lines.push('');
  lines.push("Reuse/extend the named targets. For any 'add', justify in your return why");
  lines.push('reuse was not possible. Do not re-implement a capability listed as reuse.');
  lines.push('');
  return lines.join('\n');
}

/**
 * #238: a feature-lane implementation dispatch must carry alignment on every
 * task (defense-in-depth alongside the planner validator). A dispatch is an
 * implementation dispatch when it carries a non-empty `targetAcs`; the lane
 * comes from `opts.lane`. Absent lane, or a non-feature lane → no assertion
 * (bug/chore and legacy callers stay unaffected).
 * @param {BuildDispatchPayloadOptions} opts
 * @returns {void}
 */
function assertAlignmentForFeatureImpl(opts) {
  if (opts.lane !== 'feature') return;
  if (!Array.isArray(opts.targetAcs) || opts.targetAcs.length === 0) return;
  const tasks = Array.isArray(opts.tasks) ? opts.tasks : [];
  for (const [index, task] of tasks.entries()) {
    const record = /** @type {Record<string, unknown>} */ (task);
    if (!Array.isArray(record.alignment) || record.alignment.length === 0) {
      throw new Error(
        'buildDispatchPayload: feature-lane implementation dispatch requires codebase-alignment ' +
        `evidence on every task; tasks[${index}] has none`,
      );
    }
  }
}
...
export function buildDispatchPayload(opts) {
  ...
  assertAlignmentForFeatureImpl(opts);
  ...
  const alignmentSection = buildAlignmentSection(opts.tasks);
