 ...
 *
 * The `(artifact, reason, noProjector)` triple sorts every return into exactly one of four
 * cases, and a caller keys its signalling on the combination — never on `reason` text:
 *   A. success   — `artifact !== null`, `reason === null`                → silent.
 *   B. partial   — `artifact !== null`, `reason !== null`               → the primary landed
 *      but a secondary (scope.md) was refused; actionable, surface it to the model.
 *   C. no-op     — `artifact === null`, `noProjector === true`          → this agent has no
 *      canonical artifact by design (fullstack, tech-design, ui-ux, security, spec-writer);
 *      NOT a failure, must not alert.
 *   D. failure   — `artifact === null`, `noProjector` falsy             → a projecting agent's
 *      evidence failed to validate/merge/parse; actionable, blocking.
 * A future branch that legitimately pairs a non-null artifact with a benign non-null reason
 * would be misread as case B — such a branch must add its own explicit informational flag
 * (mirror of `noProjector`) rather than lean on `reason` staying null.
 *
 ...
 * @property {string|null} reason    Why the projection did not fully succeed — either nothing was
 *   written (cases C/D) or the primary landed but a secondary was refused (case B, a partial
 *   projection). Null on a clean success (case A). An invalid return is NOT an error.
 * @property {boolean} [noProjector]  True when this agent has no projection branch at all (case C):
 *   it produces no canonical artifact by design, so an absent artifact is expected, not a failure.
...
export async function projectWorkerReturn(
  ...
  // Case C: this agent has no projection branch (fullstack, tech-design, ui-ux, security,
  // spec-writer). It produces no canonical artifact BY DESIGN, so an absent artifact here is
  // expected — not the evidence failure that an `artifact === null` from a projecting agent
  // signals. `noProjector` lets the hook tell the two apart and stay silent for this one.
  return { artifact: null, lane: null, reason: `no projection for agent "${agentName}"`, noProjector: true };
}
