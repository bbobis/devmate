 ...
 *     new task can start after an abandon. A fresh session over a terminal
 *     task bootstraps a NEW task at `no-lane`; the old task's artifacts are
 *     left in place but ignored (their `taskId` no longer matches, so every
 *     ownership-checking precondition refuses them as stale evidence).
 *     An UNREADABLE task.json is still left untouched — a state we cannot
 *     classify might be live, and clobbering it would destroy progress.
 */
...
export async function bootstrapTaskState(repoRoot, opts = {}) {
  ...
  if (pathExists(statePath)) {
    ...
    if (!existing.ok || !TERMINAL_GATES.has(existing.state.workflowGate)) {
