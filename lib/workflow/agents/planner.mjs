...
/** @typedef {import('../alignment.mjs').AlignmentDecision} AlignmentDecision */
...
/**
 * Normalize one alignment target: an object with non-empty `symbol`/`path`,
 * or `null`. Anything else collapses to `null`.
 * @param {unknown} value
 * @returns {{ symbol: string, path: string } | null}
 */
function normalizeAlignmentTarget(value) {
...
/**
 * Build a normalized alignment list. A non-object entry is dropped outright;
 * an object with missing/ill-typed fields is coerced to safe defaults (empty
 * strings/arrays) so the validator (not a crash) reports it. Normalization
 * never fabricates evidence, so a validator-invalid decision stays invalid.
 * @param {unknown} value
 * @returns {AlignmentDecision[]}
 */
function normalizeAlignment(value) {
  ...
  for (const entry of value) {
    ...
    /** @type {string} */
    const decisionRaw = typeof record.decision === 'string' ? record.decision.trim() : '';
    const reason = typeof record.reason === 'string' ? record.reason.trim() : '';
    normalized.push({
      capability,
      // A malformed/absent decision stays as its raw string for the validator
      // to reject; the cast keeps the typed shape without asserting legality.
      decision: /** @type {AlignmentDecision['decision']} */ (decisionRaw),
      target: normalizeAlignmentTarget(record.target),
      usageEvidence: normalizeStringList(record.usageEvidence),
      patternRefs: normalizeStringList(record.patternRefs),
      reason,
    });
  }
  return normalized;
...
function normalizeTasks(taskList) {
  ...
  for (const task of taskList) {
    ...
    normalized.push({ description, ac, tddApproach, persona, files, alignment });
  }
  return normalized;
}
...
export function validatePlannerArtifact(artifact) {
  ...
  if (!Array.isArray(tasks)) {
    ...
    for (const [i, task] of tasks.entries()) {
      ...
      errors.push(...alignmentErrors(taskRecord.alignment, `tasks[${i}].alignment`));
    }
  }
