...
export const AGENT_CONTRACTS = Object.freeze({
  ...
  diagnose: Object.freeze({
    ...
    example: () => ({
      ...
      allowedGlobs: [],
      // issue 240: the optional/advisory codebase-alignment contract, now
      // carried on DiagnosisResult too. A compliant diagnose return may point
      // the fix at the existing clamp to extend rather than re-implement it.
      alignment: [
        {
          capability: 'batch cursor clamp',
          decision: 'extend',
          target: { symbol: 'clampCursor', path: 'repo-a/lib/cursor.mjs' },
          usageEvidence: [],
          patternRefs: ['repo-a/lib/cursor.mjs:44'],
          reason: 'The final-page overflow is a missing branch in the existing clamp; extend it rather than add a parallel guard.',
        },
      ],
    }),
  }),
          ...
          alignment: [
            {
              ...
              usageEvidence: [],
              patternRefs: ['repo-a/lib/cursor.mjs:44'],
              reason: 'No existing guard for the absent-claim path; mirrors the clamp guard in cursor.mjs.',
            },
          ],
        },
      ],
