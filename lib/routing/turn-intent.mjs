...
import { APPROVE_PR, APPROVE_SPEC, REVISE_SPEC_PREFIX } from './approval-phrases.mjs';
 ...
 * "approve the pending human gate". Built from the single phrase source in
 * lib/routing/approval-phrases.mjs (#125) — the literals this labeller
 * recognizes and the ones `hooks/approval-listener.mjs` applies can no longer
 * drift apart.
 * @type {ReadonlySet<string>}
 */
const APPROVE_PHRASES = new Set([APPROVE_SPEC, APPROVE_PR]);

/**
 * Exact revision-phrase prefix (lower-cased), from the same single source.
 */
const REVISE_PREFIX = REVISE_SPEC_PREFIX;
