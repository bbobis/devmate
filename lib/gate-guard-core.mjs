 ...
 */
export function toPreToolUseOutput(decision) {
...
/**
 * @typedef {Object} ShellToken
 * @property {string} text  The token's literal text (quote delimiters removed).
 * @property {'word' | 'redirect' | 'separator'} kind  `redirect` marks an
 *   *unquoted* `>`/`>>` operator; `separator` marks an *unquoted* `|`/`||`/`&&`/`;`
 *   pipeline break; everything else is a `word`.
 */

/**
 * Quote-aware shell tokenizer for the source-write analyzer (#128).
 *
 * A single left-to-right pass that approximates a real shell's word-splitting:
 * words break on *unquoted* whitespace, and the only metacharacters the analyzer
 * cares about — the redirect operators `>`/`>>` and the segment separators
 * `|`/`||`/`&&`/`;` — are emitted as their own tokens *only when unquoted*.
 * Everything inside single or double quotes is literal, so a `>` or `;` sitting
 * inside quotes stays part of its word and is never mistaken for an operator.
 * That is what stops an ordinary `git commit -m "renamed > foo.mjs"` or
 * `grep "> some/path.mjs" f` from reading as a redirect onto a source file.
 *
 * A single `&` (as in `2>&1`) is an ordinary word character — only `&&` breaks a
 * segment — which preserves file-descriptor duplication, matching the operator
 * set of the splitter this replaced. Backslashes are left literal so Windows
 * paths (`.devmate\session\spec.md`) survive intact for {@link isWriteTargetToken}
 * to normalize. Quote delimiters are consumed, not emitted; an unclosed quote
...
function tokenizeShellCommand(command) {
  ...
  const flushWord = () => {
    if (inWord) {
      tokens.push({ text: cur, kind: 'word' });
      cur = '';
      inWord = false;
    }
  };
  ...
  for (let i = 0; i < command.length; i++) {
    ...
    if (ch === "'" || ch === '"') {
      quote = ch;
      inWord = true; // an empty quoted string is still a token
      continue;
    }
    if (ch === ' ' || ch === '\t' || ch === '\n' || ch === '\r' || ch === '\f' || ch === '\v') {
      flushWord();
    ...
    if (ch === '>') {
      ...
      const op = next === '>' ? '>>' : '>';
      if (op === '>>') i++;
      tokens.push({ text: op, kind: 'redirect' });
      continue;
    }
    if (ch === '|') {
      flushWord();
      const op = next === '|' ? '||' : '|';
      if (op === '||') i++;
      tokens.push({ text: op, kind: 'separator' });
      continue;
    }
    if (ch === ';') {
      flushWord();
    ...
    // Any other character (including a lone `&`) is part of the current word.
    cur += ch;
    inWord = true;
  }
  flushWord();
 ...
 * write a source file:
 *  - any *unquoted* `>`/`>>` redirect whose target has a source extension;
 *  - the in-place editor family (`sed -i`, `perl -i`, `git apply`, `patch`,
 *    `tee` onto a source path, `mv`/`cp` onto a source path);
 *  - opaque inline interpreters (`python -c`, `python3 -c`, `node -e`) — their
 *    embedded code cannot be analyzed, so they fail closed;
 ...
 *
 * Tokenization is quote-aware ({@link tokenizeShellCommand}, #128): a `>` or a
 * source-path-looking substring that lives inside quotes is literal text, not a
 * redirect, and `tee` is a write only when its actual argument names a write
 * target — so `git commit -m "renamed > foo.mjs"` and
 * `npm test 2>&1 | tee test-results.log` are no longer misclassified, while the
 * fail-closed guarantee for real writes is unchanged.
 * @param {string} command
 * @returns {boolean}
 */
function shellWritesSource(command) {
  ...
  // 1) Any *unquoted* redirect (`>`, `>>`, incl. the `>` of a `2>` where the fd
  //    digit is a separate token) onto a source-extension target or a session
  //    artifact, whatever command produced the output. A quoted `>` is a `word`
  //    token, never a `redirect`, so it cannot match here (#128).
  for (let i = 0; i < tokens.length - 1; i++) {
    const kind = tokens[i]?.kind;
    if (kind === 'redirect' && isWriteTargetToken(tokens.at(i + 1)?.text ?? '')) {
      return true;
    }
  }

  // PowerShell write cmdlets anywhere in the command line.
  if (/(^|\s|\|)\s*(set-content|out-file|add-content)\b/i.test(command)) return true;

  // 2) Analyze each pipeline segment independently so `... | tee file.mjs`
  //    and `... | patch` are caught regardless of the first command. Segments
  //    are split on the *unquoted* separator tokens only.
  /** @type {string[][]} */
  const segments = [];
  /** @type {string[]} */
  let segment = [];
  ...
  for (const segTokens of segments) {
    ...
    // `tee` writes only when its actual argument names a write target — mirror
    // the `mv`/`cp` check below rather than flag every capture-to-log (#128).
  ...
  //    session artifact is treated as an edit.
  if (sawUnknownSegment && tokens.some((t) => isWriteTargetToken(t.text))) return true;
 ...
 * @param {{ sessionArtifactPaths?: readonly string[], sessionArtifactWriters?: readonly SessionArtifactWriter[], activeAgent?: string, activeAgentAmbiguous?: boolean, scope?: ParsedScope, budgetCritical?: import('./types.mjs').BudgetCriticalMarker|null }} [opts]
  ...
  if (state !== null && isEdit && !isEditAllowedAtGate(state.workflowGate)) {
    ...
    return {
      ...
      reason:
        ...
        'Do not edit source directly — delegate the change to @fullstack.',
    };
  }
