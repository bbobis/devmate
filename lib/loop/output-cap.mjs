...
function secretPatterns() {
  return [
    ...
    // Bare provider tokens not in KEY=value form (#222). Each keeps its
    // identifying prefix (group 1) and redacts the secret body (group 2), so the
    // shared callback renders e.g. `ghp_[REDACTED]`.
    // GitHub PATs: ghp_/gho_/ghs_/ghu_/ghr_ classic + github_pat_ fine-grained.
    /\b(gh[oprsu]_|github_pat_)([A-Za-z0-9_]{20,})/g,
    // AWS access key id: AKIA/ASIA + 16 upper/digits.
    /\b(AKIA|ASIA)([0-9A-Z]{16})\b/g,
    // Slack tokens: xoxb-/xoxp-/xoxa-/xoxr-/xoxs- + body.
    /\b(xox[abprs]-)([A-Za-z0-9-]{10,})/g,
    // PEM private-key block (any key type), header kept, body+footer redacted.
 ...
 * Replaces matched secret values with [REDACTED]. Covers env-var `KEY=value`,
 * `Bearer …`, `Authorization: …`, long delimited base64, and bare provider
 * tokens (GitHub `ghp_`/`github_pat_`, AWS `AKIA`/`ASIA`, Slack `xox*-`) and PEM
 * private-key blocks (#222). Best-effort — not a security guarantee.
 * @param {string} text
 * @returns {string}
 */
