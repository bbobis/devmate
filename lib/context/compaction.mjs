...
export async function compactAndReclaim(opts) {
  // Point the builder at the CANONICAL per-task trace directory, derived from
  // the task-state path it already anchors on. Without this, the builder fell
  // back to the flat `.devmate/state/trace.jsonl` default — a file the runtime
  // never writes (traces live at `.devmate/state/trace/<taskId>.jsonl`) — so
  // every production artifact's nextAction was the generic fallback and its
  // trace-derived context (decisions, unresolved halts) was always empty (#22).
  const artifact = await buildCompactionArtifact({
    taskStatePath: opts.taskStatePath,
    traceDir: join(dirname(opts.taskStatePath), 'trace'),
  });
