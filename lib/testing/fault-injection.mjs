 ...
 *
 * Parses the strict grammar `"<site>:<mode>"` (a single colon). A value that
 * names a different site, an unknown mode, or does not parse returns null — the
 * seam does nothing. No dynamic `RegExp`, no table indexed by a runtime string:
 * only `String.prototype` ops and frozen-Set membership, so the security lint has
 * nothing to flag.
 *
...
export function armedFaultFor(site, env = process.env) {
  ...
  if (!FAULT_SITES.has(specSite)) return null;
