 ...
 *
 * #150: the committed MEMORY.md renders SEMANTIC discovery facts only, and the
 * doctor's render invariant now compares the discovery-fact subset against the
 * rendered lines — so the seeded fact must be a discovery-merge fact to model a
 * healthy pipeline (a bare edit event would render nothing and read as broken).
 * @param {string} root
 */
function seedHealthyMemory(root) {
  ...
  writeFileSync(
    ...
    `${JSON.stringify({ event: 'fact', ts: 1, tool: 'discovery-merge', source: 'lib/seed.mjs', summary: 'seed fact', confidence: 0.9, text: 'seed fact' })}\n`,
    'utf8',
  );
  writeFileSync(
