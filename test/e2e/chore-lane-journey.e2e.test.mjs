 ...
 * an executor step with no hook), and the two tail transitions
 * (`pass-verification`, `complete`) are driven with `transitionGate` exactly as
 * a lane executor would — those turns are called out inline.
 */
...
describe('E2E — chore lane: the full journey from no-lane to done (mechanical, no human gate)', () => {
   ...
   * no hook caller), captured so the contiguity assertion can prove the FULL
   * journey has no gaps, not merely the hook-recorded slice of it.
  ...
  it('gate verification-passed (executor-driven, not hook-fired) — reached on fresh, passing evidence', () => {
