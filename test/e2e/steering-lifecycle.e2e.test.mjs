 ...
 *
 * Two runtime callers exist. The `gatectl workflow set <event>` CLI
 * (docs/gates.md § Steering transitions and § Subcommand syntax) drives every
 * edge in the suites below: each spawns the real `scripts/gatectl.mjs` as a
 * subprocess with cwd = the workspace's own `.devmate/` folder — the same
 * doubled-path hazard the hooks face (#76) — and the oracle is what landed on
 * disk, never the CLI's word. Since #127 the mid-implementation edges are ALSO
 * fired by the human phrases "revise scope: <reason>" / "re-plan: <reason>"
 * through the UserPromptSubmit approval listener — the final describe drives
 * those through the real prompt-hook subprocess.
 *
...
// ---------------------------------------------------------------------------
// #127: the HUMAN PHRASES "revise scope:" / "re-plan:" wire steerFeature
// through the real UserPromptSubmit hook — before this wiring, steerFeature
// had no runtime caller at all.
// ---------------------------------------------------------------------------

describe('E2E steering — the steering phrases drive steerFeature from a real prompt (#127)', () => {
  /** @type {ReturnType<typeof seedMonorootWorkspace>} */
  let ws;
  /** @type {string} */
  let taskId;
  ...
  it('"revise scope: <reason>" captures the note ITSELF and returns to spec-draft', () => {
    const runs = replaySession(
      [{ hook_event_name: 'UserPromptSubmit', session_id: SESSION_ID, prompt: 'revise scope: also handle CSV export' }],
      ws.hostCwd,
    );
    for (const r of runs) {
    ...
    const state = readState(ws.root);
    assert.equal(state.workflowGate, 'spec-draft', 'the phrase did not steer to spec-draft');
    assert.equal(state.taskId, taskId, 'steering restarted the task instead of continuing it');

    // The hook wrote the precondition artifact from the phrase — no separate
    // capture step was needed (the phrase IS the note).
    const note = JSON.parse(readFileSync(stateArtifact(ws, 'scope-change.json'), 'utf8'));
    assert.equal(note.taskId, taskId);
    assert.equal(note.note, 'also handle CSV export');

    // The audited trace event binds the verbatim phrase as evidence.
    const transition = readTraceEvents(tracePath(ws, taskId)).find(
      (e) => e.type === 'gate_transition' && e.to === 'spec-draft' && e.from === 'impl-started',
    );
    assert.ok(transition, 'no audited gate_transition for the steering move');
    assert.equal(transition?.evidence, 'revise scope: also handle CSV export');
  });

  it('re-approval reopens implementation on the same task', () => {
    replaySession(
      [{ hook_event_name: 'UserPromptSubmit', session_id: SESSION_ID, prompt: 'approve spec' }],
      ws.hostCwd,
    );
  ...
  it('"re-plan: <reason>" returns to plan-done on the same task', () => {
    const runs = replaySession(
      [{ hook_event_name: 'UserPromptSubmit', session_id: SESSION_ID, prompt: 're-plan: switch to a queue-based approach' }],
      ws.hostCwd,
    );
    for (const r of runs) {
      assert.equal(r.status, 0, `${r.script} exited ${r.status}:\n${r.stdout}${r.stderr}`);
    ...
    const state = readState(ws.root);
    assert.equal(state.workflowGate, 'plan-done', 'the phrase did not steer to plan-done');
    assert.equal(state.taskId, taskId);
  });

  it('a steering phrase at the wrong gate refuses with a model-visible reason, exit 0', () => {
    // The task now rests at plan-done — revise-scope only fires from impl-started.
    const runs = replaySession(
      [{ hook_event_name: 'UserPromptSubmit', session_id: SESSION_ID, prompt: 'revise scope: too late' }],
      ws.hostCwd,
    );
    for (const r of runs) {
      assert.equal(r.status, 0, `a refused steering phrase must not crash the chat: ${r.stderr}`);
    }
    assert.equal(readState(ws.root).workflowGate, 'plan-done', 'the gate moved on a refused steering phrase');
    const withOutput = runs.find((r) => r.stdout.includes('did not move the gate'));
    assert.ok(withOutput, 'the refusal must reach the model-visible channel');
  });
});
