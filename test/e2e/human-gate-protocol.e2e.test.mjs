...
const TEST_SESSION_ID = markSessionForFile('devmate-test-human-gate');
...
describe('E2E — human gate protocol: rejection & revision loops', { concurrency: false }, () => {
  it('three revise-spec cycles keep the gate at spec-draft and record verbatim feedback', () => {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      // #126: the feedback must reach the model on the SAME turn through the
      // real subprocess envelope — a trace-file write alone is invisible.
      const visible = additionalContextOf(result);
      assert.ok(
        visible.includes(feedback),
        `cycle ${i}: model-visible output must carry the verbatim feedback`,
      );
      assert.ok(
        visible.includes('@spec-writer'),
        `cycle ${i}: model-visible output must instruct a @spec-writer redraft`,
      );
    }
