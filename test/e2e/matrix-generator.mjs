...
import { APPROVE_PR, approvalPhraseForGate } from '../../lib/routing/approval-phrases.mjs';
...
function expectedPhraseOutcome(lane, gate, event, ctx) {
  ...
  // approve-pr (#176): the lane-aware handler accepts it ONLY where it is the
  // lane's designated phrase — approvalPhraseForGate === APPROVE_PR, i.e.
  // verification-passed on feature/bug. The chore lane completes at
  // verification-passed and the guard refuses the walk into pr-ready that the
  // FLATTENED table used to allow (the #125 anchor already refused to advertise
  // it there; #176 makes the executing surface agree).
  if (gate === 'pr-ready') {
    return { kind: 'no-move', mustMention: 'already marked ready' };
  }
  if (approvalPhraseForGate(gate, lane) === APPROVE_PR) {
    return { kind: 'advance', to: /** @type {WorkflowGate} */ ('pr-ready') };
  }
  // chore/verification-passed is the walk-hazard the #176 guard intercepts with a
  // lane-specific message; every OTHER gate never legally reaches pr-ready, so it
  // falls through to advanceHumanGate's consistent "did not advance" refusal.
  if (lane === 'chore' && gate === 'verification-passed') {
    return { kind: 'no-move', mustMention: 'never enters pr-ready' };
  }
  return { kind: 'no-move', mustMention: 'did not advance' };
}
