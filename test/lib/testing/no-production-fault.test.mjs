...
/** #202 — the CAS-conflict seam's env var and its single defining file. */
const CAS_CONFLICT_VAR = 'DEVMATE_CAS_CONFLICT';
const CAS_CONFLICT_SEAM_FILE = join('lib', 'testing', 'cas-conflict-seam.mjs');
...
test('no production file outside its seam even names the CAS-conflict env var (#202)', () => {
  /** @type {string[]} */
  const offenders = [];
  for (const root of PRODUCTION_ROOTS) {
    for (const rel of mjsFilesUnder(root)) {
      if (relative(CAS_CONFLICT_SEAM_FILE, rel) === '') continue; // the seam itself is allowed
      // eslint-disable-next-line secure-coding/no-unlimited-resource-allocation -- bounded by the repo's own committed source tree, not by any runtime/user input.
      const text = readFileSync(join(REPO_ROOT, rel), 'utf8');
      if (text.includes(CAS_CONFLICT_VAR)) offenders.push(rel);
    }
  }
  assert.deepEqual(
    offenders,
    [],
    `production code must never reference ${CAS_CONFLICT_VAR}; found in: ${offenders.join(', ')}`,
  );
});

test('the CAS-conflict seam reads its var but never assigns it (#202)', () => {
  const text = readFileSync(join(REPO_ROOT, CAS_CONFLICT_SEAM_FILE), 'utf8');
  assert.ok(text.includes(CAS_CONFLICT_VAR), 'the seam is expected to define the var name');
  for (const line of text.split('\n')) {
    if (!line.includes(CAS_CONFLICT_VAR)) continue;
    assert.doesNotMatch(
      line,
      /process\.env\[[^\]]*\]\s*=|process\.env\.DEVMATE_CAS_CONFLICT\s*=/,
      `the seam must not assign the CAS-conflict var: ${line.trim()}`,
    );
  }
});
