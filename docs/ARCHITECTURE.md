...
**Runtime session-scoping (not frontmatter):** Because plugin-level hooks fire in *every* Copilot session — including sessions that never involve devmate — devmate scopes its behavior at runtime rather than through per-agent frontmatter. (Agent-frontmatter `hooks:` were evaluated and rejected: the host does not expand `${PLUGIN_ROOT}` in frontmatter and runs frontmatter commands with cwd = the workspace, so a plugin-shipped agent cannot resolve a path to its own scripts — both forms verified failing in-host.)

A session is treated as "devmate" only once a devmate agent has been dispatched. The first devmate `SubagentStart` — gated by the frozen roster in [`lib/agents/roster.mjs`](../lib/agents/roster.mjs) — writes a per-`session_id` marker into the OS temp dir ([`lib/hooks/session-marker.mjs`](../lib/hooks/session-marker.mjs); activity-refreshed, 7-day TTL, no clear-on-`Stop` because VS Code reuses a chat `session_id` across reloads and `Stop` fires per turn). Every state-writing or blocking handler checks the marker **first** and, when it is absent, exits inert — `gate-guard` emits `allow` before reading any `task.json`; `post-tool-use`, `gate-advance`, `spec-integrity-guard`, `contract-validator`, and `check-session-budget` return without writing or injecting context. So outside a devmate session devmate never blocks a tool call, writes `.devmate/state`, or adds to the model's context.

The one deliberate exception: an **implementation dispatch** (`runSubagent` targeting `fullstack` or a persona-wrapper) is gate-checked even before the marker exists. It self-identifies as devmate — no plain-Copilot session dispatches an agent by those names — and it is the highest-risk gate, so it is denied at dispatch time on the session's very first dispatch (closing the bootstrap gap that would otherwise let a worker start at a pre-implementation gate). Handlers left unscoped by design: `session-start` (already inert without a `.devmate/devmate.config.json`), `approval-listener` (its `<devmate-skills>` menu is the bootstrap/discovery surface), and `compact-session` (reads no stdin and doubles as the user-run budget-critical remedy).

This scoping is independent of the `chat.useCustomAgentHooks` preview setting, which gates *frontmatter* hooks only — devmate's plugin-level hooks fire regardless (verified in-host with the setting unset).
