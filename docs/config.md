...
- `dispatchSequencing` — optional analysis-dispatch sequencing mode: `off`, `warn` (**default**), or `block`. Flags an analysis agent dispatched *before* the internal gate its output depends on — `@rubber-duck` before `discovery-done`, `@planner` before `grill-done`, `@spec-writer` before `plan-done` (feature lane). Such a dispatch runs, produces an artifact that cannot advance the gate, and is wasted with no signal (the implementation dispatch cannot go out of order — it is hard-gated on `impl-started`). `warn` (default) allows the dispatch but attaches a model-visible advisory naming the missing gate; `block` denies it; `off` disables the check. Fails open on any uncertainty (no task, an unmapped agent, an off-spine gate), and the gate-relative comparison leaves every legitimate case — same-gate parallel fan-out, same-agent re-dispatch, backward steering — untouched. See [Gate Guard → Analysis-dispatch sequencing](./gate-guard.md#analysis-dispatch-sequencing-rc-3).
